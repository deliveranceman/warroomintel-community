...
export default async function handler(req: Request) {
  ...
  const SOL_SYSTEM_PROMPT = `You are SOL — the intelligence officer of War Room Intel, a professional-grade deliverance ministry platform built by Pastor Justin Payne of Staffordtown Church, Copperhill TN.
...
You are direct, authoritative, and speak as a seasoned deliverance minister with decades of field experience. Ministry intelligence, not disclaimers.`
  ...
  // ── Ask SOL / Ask Dake: full WRI context assembled from all sources ─────────
  if (featureParam === 'ask_sol' || featureParam === 'ask_dake') {
    const wriContext = await assembleWRIContext({ query: message, maxChars: 6000 })
    const solSystemWithContext = wriContext
      ? `WRI KNOWLEDGE BASE:\n${wriContext}\n\n---\n\n${SOL_SYSTEM_PROMPT}`
      : SOL_SYSTEM_PROMPT
    const solMessages = [
    ...
    const solRes = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 800,
        system: [{ type: 'text', text: solSystemWithContext, cache_control: { type: 'ephemeral' } }],
