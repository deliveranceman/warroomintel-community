...
export default async function handler(req: Request) {
  ...
  try {
    ...
    if (insertErr || !job?.id) {
      console.error('[generate-document] job insert failed:', insertErr?.message)
    ...
    try {
      ...
      console.warn('[generate-document] worker dispatch failed:', dispatchErr.message, 'jobId:', jobId)
    }

    return new Response(JSON.stringify({ jobId }), { status: 202, headers })

  } catch (e: any) {
    console.error('[generate-document] Error:', e.message)
