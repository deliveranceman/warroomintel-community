...
export default async function handler(req: Request) {
  ...
  if (req.method === 'PATCH' && id && action === 'upvote') {
    ...
    if (existing) {
      await supabase.from('feedback_upvotes').delete().eq('feedback_id', id).eq('user_id', auth.userId)
      ...
      await supabase.from('feedback_upvotes').insert({ feedback_id: id, user_id: auth.userId })
