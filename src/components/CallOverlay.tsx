...
export default function CallOverlay({ callType, otherUser, onEnd }: CallOverlayProps) {
  ...
  useEffect(() => {
    ...
    const start = async () => {
      try {
        ...
        pc.onconnectionstatechange = () => { if (pc.connectionState === 'failed') setStatus('error') }

        setStatus('active')
        startRef.current = Date.now()
        timerRef.current = setInterval(() => setElapsed(Math.floor((Date.now() - startRef.current) / 1000)), 1000)
      } catch (err) {
        console.error('[CallOverlay]', err)
  ...
  const card: React.CSSProperties = {
    width: '100%', maxWidth: 400, background: '#0a0d1a',
    borderRadius: 16, overflow: 'hidden', border: '1px solid rgba(201,168,76,0.2)',
  }
    ...
    <div style={overlay}>
      <div style={card}>
        ...
        <div style={{ padding: '14px 24px 18px', display: 'flex', justifyContent: 'center', gap: 14, alignItems: 'center', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
          <button
            onClick={() => { const t = localStreamRef.current?.getAudioTracks()[0]; if (t) { t.enabled = !t.enabled; setMuted(m => !m) } }}
            style={btn(muted)}
          >{muted ? '🔇' : '🎙'}</button>
          {callType === 'video' && (
            <button
              onClick={() => { const t = localStreamRef.current?.getVideoTracks()[0]; if (t) { t.enabled = !t.enabled; setCameraOff(c => !c) } }}
              style={btn(cameraOff)}
            >{cameraOff ? '📵' : '📹'}</button>
          )}
